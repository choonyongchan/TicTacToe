"""Tests for tictactoe/agents/minimax_rewards_alphabeta_agent.py."""
...
class TestMinimaxRewardsSmallTree:
    ...
    def test_winning_branch_scores_depth7(self):
        ...
        agent = MinimaxRewardsAlphaBetaAgent(Player.X, max_depth=9)
        assert state.is_terminal()
        assert agent._minimax(state, float("-inf"), float("inf")) == pytest.approx(
            1.0 - _EPS * 7
        )
    ...
    def test_maximize_picks_best_child(self):
        ...
        state = state_with_moves(_DUMMY_TREE_MOVES)
        agent = MinimaxRewardsAlphaBetaAgent(Player.X, max_depth=9)
        assert agent._maximize(state, float("-inf"), float("inf")) == pytest.approx(
            1.0 - _EPS * 7
        )
    ...
    def test_minimize_at_losing_branch(self):
        ...
        state.apply(0, 2)
        agent = MinimaxRewardsAlphaBetaAgent(Player.X, max_depth=9)
        assert agent._minimize(state, float("-inf"), float("inf")) == pytest.approx(
            -1.0 + _EPS * 8
        )
...
class TestTwoAgentsDraw:
    ...
    def test_two_agents_draw(self):
        agent_x = MinimaxRewardsAlphaBetaAgent(Player.X, max_depth=9)
        agent_o = MinimaxRewardsAlphaBetaAgent(Player.O, max_depth=9)
