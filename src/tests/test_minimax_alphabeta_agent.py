...
#   X | X | .
#   O | . | O
# It is X's turn. Maximizer = X. Empty cells (in order): (0,2), (1,2), (2,1)
#
# Hand-traced scores (maximizer=X):
#   X(0,2) → O's best response → score = -1
#   X(1,2) → row 1 = X,X,X → X WINS immediately → score = 1
#   X(2,1) → O's best response → score = 0
