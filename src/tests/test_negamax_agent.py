...
class TestInit:
    def test_name(self):
        assert NegamaxAgent(Player.X).name == "NegamaxAgent"

    def test_maximizer_stored(self):
        assert NegamaxAgent(Player.O).maximizer == Player.O

    def test_default_epsilon(self):
        assert NegamaxAgent(Player.X)._epsilon == pytest.approx(1.0 / 10)

    def test_custom_max_depth(self):
        assert NegamaxAgent(Player.X, max_depth=15)._epsilon == pytest.approx(1.0 / 16)


class TestTerminalScore:
    """_terminal_score matches MinimaxRewards: win>0, loss<0, draw=0."""

    def test_win_at_depth_5(self):
        state = state_with_moves([(0, 0), (1, 0), (0, 1), (1, 1), (0, 2)])
        agent = NegamaxAgent(Player.X)
    ...
    def test_draw_returns_zero(self):
        ...
        assert NegamaxAgent(Player.X)._terminal_score(state) == 0.0

    def test_win_score_is_positive(self):
        state = state_with_moves([(0, 0), (1, 0), (0, 1), (1, 1), (0, 2)])
        assert NegamaxAgent(Player.X)._terminal_score(state) > 0
    ...
    def test_custom_max_depth_scales_epsilon(self):
        state = state_with_moves([(0, 0), (1, 0), (0, 1), (1, 1), (0, 2)])
        agent = NegamaxAgent(Player.X, max_depth=19)
        assert agent._terminal_score(state) == pytest.approx(1.0 - (1.0 / 20) * 5)


class TestNegamaxSmallTree:
    """Hand-traced depth-scaled scores on the dummy search tree (maximizer=X)."""

    def test_winning_branch_scores_depth7(self):
        # X(1,2) → terminal, X wins row 1, depth 7 → 1.0 - ε×7
        state = state_with_moves(_DUMMY_TREE_MOVES + ((1, 2),))
        agent = NegamaxAgent(Player.X)
        assert state.is_terminal()
        assert agent._negamax(state, float("-inf"), float("inf")) == pytest.approx(1.0 - _EPS * 7)

    def test_losing_branch_scores_neg_depth8(self):
        # X(0,2) → O minimises to O(2,1) win at depth 8 → -1.0 + ε×8
        state = state_with_moves(_DUMMY_TREE_MOVES)
        state.apply(0, 2)
        agent = NegamaxAgent(Player.X)
        assert agent._negamax(state, float("-inf"), float("inf")) == pytest.approx(-1.0 + _EPS * 8)
    ...
    def test_root_score_is_winning_path(self):
        # max(0.3, -0.2, 0.0) = 1.0 - ε×7
        state = state_with_moves(_DUMMY_TREE_MOVES)
        agent = NegamaxAgent(Player.X)
        assert agent._negamax(state, float("-inf"), float("inf")) == pytest.approx(1.0 - _EPS * 7)

    def test_minimize_at_draw_branch(self):
        # After X(2,1): O sees scores 1.0-ε×9 and 0.0 → minimise = 0.0
        state = state_with_moves(_DUMMY_TREE_MOVES)
        state.apply(2, 1)
        agent = NegamaxAgent(Player.X)
        assert agent._negamax(state, float("-inf"), float("inf")) == pytest.approx(0.0)
    ...
    def test_minimize_at_losing_branch(self):
        # After X(0,2): O sees -1.0+ε×8 and 0.0 → minimise = -1.0+ε×8
        state = state_with_moves(_DUMMY_TREE_MOVES)
        state.apply(0, 2)
        agent = NegamaxAgent(Player.X)
        assert agent._negamax(state, float("-inf"), float("inf")) == pytest.approx(-1.0 + _EPS * 8)
...
class TestActWinningMove:
    ...
    def test_wins_in_one_row(self):
        ...
        assert NegamaxAgent(Player.X).act(state) == (0, 2)

    def test_wins_in_one_column(self):
        state = state_with_moves([(0, 0), (0, 2), (1, 0), (1, 2)])
        assert NegamaxAgent(Player.X).act(state) == (2, 0)

    def test_picks_winning_move_on_dummy_tree(self):
        state = state_with_moves(_DUMMY_TREE_MOVES)
        assert NegamaxAgent(Player.X).act(state) == (1, 2)
...
class TestActBlockingMove:
    ...
    def test_blocks_opponent_diagonal_win(self):
        ...
        assert NegamaxAgent(Player.X).act(state) == (2, 2)
...
class TestActLargerBoards:
    ...
    def test_4x4_picks_best_move(self):
        ...
        agent = NegamaxAgent(Player.X, max_depth=PUZZLE_4X4.n ** 2)
    ...
    def test_5x5_picks_best_move(self):
        ...
        agent = NegamaxAgent(Player.X, max_depth=PUZZLE_5X5.n ** 2)
...
class TestAgreesWithMinimaxRewards:
    ...
    def _minimax(self, player: Player, max_depth: int = 9) -> MinimaxRewardsAlphaBetaAgent:
        return MinimaxRewardsAlphaBetaAgent(player, max_depth=max_depth)

    def test_agrees_on_dummy_tree(self):
        state = state_with_moves(_DUMMY_TREE_MOVES)
        assert NegamaxAgent(Player.X).act(state) == self._minimax(Player.X).act(state)
    ...
    def test_agrees_on_4x4_puzzle(self):
        state = state_with_moves(PUZZLE_4X4.moves, PUZZLE_4X4.n, PUZZLE_4X4.k)
        md = PUZZLE_4X4.n ** 2
        assert NegamaxAgent(Player.X, max_depth=md).act(state) == self._minimax(Player.X, md).act(state)
    ...
    def test_two_negamax_agents_draw(self):
        state = fresh_state()
        agent_x = NegamaxAgent(Player.X)
        agent_o = NegamaxAgent(Player.O)
