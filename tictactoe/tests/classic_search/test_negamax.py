"""Tests for the NegaMax classic search agent."""
...
class TestNegaMaxNeverLoses:
    def test_never_loses_to_oracle_n3(self) -> None:
        """NegaMax should never lose to BruteForceOracle on 3x3."""
        agent = NegaMax(depth=4)
        ...
        assert result.agent_b_wins == 0, (
            f"NegaMax lost {result.agent_b_wins} games to BruteForceOracle"
        )


class TestNegaMaxForcedMoves:
...
class TestNegaMaxInstrumentation:
...
class TestNegaMaxDeterminism:
    def test_same_state_same_move(self) -> None:
        """NegaMax is deterministic: same state must return same move."""
        agent = NegaMax(depth=4)
