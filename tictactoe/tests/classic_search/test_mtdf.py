"""Tests for the MTDf classic search agent."""
...
class TestMTDfNeverLoses:
    def test_never_loses_to_oracle_n3(self) -> None:
        """MTDf should never lose to BruteForceOracle on 3x3."""
        agent = MTDf(depth=4)
        ...
        assert result.agent_b_wins == 0, (
            f"MTDf lost {result.agent_b_wins} games to BruteForceOracle"
        )


class TestMTDfForcedMoves:
...
class TestMTDfDeterminism:
    def test_same_state_same_move(self) -> None:
        """MTDf is deterministic: same state must return same move."""
        agent = MTDf(depth=4)
