"""Tests for the MinimaxAB classic search agent."""
    ...
    for r, row_str in enumerate(board_rows):
        for c, ch in enumerate(row_str):
            if ch == 'X':
                ...
                board[r][c] = Cell.O
    # Find any last move (for is_terminal calls).
    last_move = None
    for r in range(n):
        for c in range(n):
...
class TestMinimaxABNeverLoses:
    def test_never_loses_to_oracle_n3(self) -> None:
        """MinimaxAB should never lose to BruteForceOracle on 3x3."""
        agent = MinimaxAB(depth=4)
        ...
        assert result.agent_b_wins == 0, (
            f"MinimaxAB lost {result.agent_b_wins} games to BruteForceOracle"
        )


class TestMinimaxABForcedMoves:
...
class TestMinimaxABLegality:
    def test_returns_legal_move(self) -> None:
        ...
        move = agent.choose_move(state)
        row, col = move
        assert 0 <= row < state.n, "Row out of bounds"
        assert 0 <= col < state.n, "Col out of bounds"
        assert state.board[row][col] is Cell.EMPTY, "Cell is not empty"
...
class TestMinimaxABInstrumentation:
...
class TestMinimaxABDeterminism:
    def test_same_state_same_move(self) -> None:
        """MinimaxAB is deterministic: same state must return same move."""
        agent = MinimaxAB(depth=4)
...
class TestMinimaxVsNegaMaxPrunings:
    def test_minimax_negamax_equal_prunings(self) -> None:
        """MinimaxAB and NegaMax should produce equal or comparable prunings on the same state."""
        ab_agent = MinimaxAB(depth=4)
        negamax_agent = NegaMax(depth=4)
        state_ab = make_state(['XO.', '.X.', '..O'], player=Player.X, k=3)
        state_nm = make_state(['XO.', '.X.', '..O'], player=Player.X, k=3)

        ab_agent.choose_move(state_ab)
        negamax_agent.choose_move(state_nm)

        # Both should have positive prunings on this mid-game position.
        assert state_ab.prunings > 0
        assert state_nm.prunings > 0
